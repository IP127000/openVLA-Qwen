...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    for i, source in enumerate(sources):
        ...
        for conv in source:
            ...
            # print('type of encode_id:',type(encode_id))
...
def preprocess_qwen_vla(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    # roles = {"human": "<|im_start|>user", "gpt": "<|im_start|>assistant"}
    roles = {"human": "user", "gpt": "assistant"}
    ...
    for i, source in enumerate(sources):
        ...
        for conv in source:
            ...
            # 动作编码
            if role == "assistant":
                action_id = action_tokenizer(np.array(content))
                conv = [{"role" : role, "content" : str(content)}]
                rendered_text = tokenizer.apply_chat_template(conv,tokenize=False)
                # 占位符
                placeholder = '[RPL]'
                rendered_text_modified = rendered_text.replace(str(content), placeholder)
                tokenized_output = tokenizer(rendered_text_modified,add_special_tokens=False,return_tensors='pt')
                temp = tokenized_output['input_ids']
                # 获取占位符的 token IDs
                ...
                main_seq = temp.squeeze().tolist()
                sub_seq = placeholder_ids

                idx = find_subsequence(main_seq, sub_seq)
                if idx == -1:
                    raise ValueError("Placeholder not found in input_ids")

                #action_id是一个ndarray，需要转换成list
                new_input_ids = action_id.tolist()

                # print(type(new_input_ids))
                # print('new_input_ids:',new_input_ids)
                # print(main_seq[:idx])
                # print(main_seq[idx+len(sub_seq):])

                # 拼接三个列表成一个列表

                encode_id = main_seq[:idx] + new_input_ids + main_seq[idx+len(sub_seq):]
                # print('encode_id:',encode_id)
...
def preprocess(sources: Sequence[str], tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False) -> Dict:
    ...
    """
    if conversation_lib.default_conversation.version == "qwen_vla":
        # print("Using qwen_vla preprocess")
        return preprocess_qwen_vla(sources, tokenizer, has_image=has_image)
...
class LazySupervisedDataset(Dataset):
    ...
    def modality_lengths(self):
        ...
        for sample in self.list_data_dict:
            ...
            # 有可能是列表，也有可能是字符串，要改一下
            cur_len = 0
            for conv in sample["conversations"]:
                if isinstance(conv['value'], list):
                    cur_len += len(conv['value'])
                else:
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        # model.initialize_action_tokenizer(model_args, actiontokenizer=ActionTokenizer(tokenizer))
    ...
    # 探针，看看这个时候embedding还是不是0
    with open("embedding.txt", "w") as f:
        f.write(str(model.get_input_embeddings().weight.size()))
        f.write(str(model.get_output_embeddings().weight.size()))
        f.write(str(model))
